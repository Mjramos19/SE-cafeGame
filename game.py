...
class GameManager:
    ...
    def get_nearby_seated_customer(self, player, customers):
        """Return the first seated customer close enough to serve. Uses the same general 'nearby interaction' style as machines."""
    ...
    def front_view_rendering(self, player, customers, font, keys, DebugMode):
        ...
        if player.rect.bottom < 610:
            ...
            for c in customers:
                ...
                if c.orderedItem is not None and c.state == "finding seat":
...
def main():
    ...
    player.inventory[0] = [Cup(["cup", "cup_w_lid"], [espresso_shot])]
    is_dragging = False
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_e:
                    ...
                    if player.get_foot_rect().colliderect(register1.interactionZone) and register1.customerWaiting and CafeView == "FRONT":
                        ...
                        GameState = "REGISTER"
                    elif player.get_foot_rect().colliderect(doorEntry) and CafeView == "MIDDLE":
                        CafeView = "BACKROOM"
